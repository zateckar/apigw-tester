...
func TestBaselineProbeIsSevenFixedSpecs(t *testing.T) {
	ids := NewIDTracker(120)
	probes := BuildBaselineProbe(ids)
	if len(probes) != 7 {
		t.Fatalf("want 7 probes, got %d", len(probes))
	}
	classes := map[string]bool{}
	for _, p := range probes {
		classes[p.Class] = true
		if p.ExpectInvalid && p.Class != "invalid" {
			t.Fatalf("probe %s expectInvalid without invalid class", p.Class)
		}
	}
	for _, want := range []string{"small-rest", "soap", "big-response", "big-request", "slow-upstream", "concurrency", "invalid"} {
		if !classes[want] {
			t.Fatalf("missing baseline class %s", want)
		}
	}
}
